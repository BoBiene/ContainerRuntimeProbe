...
internal static class Classifier
{
    public static ReportClassification Classify(IReadOnlyList<ProbeResult> probes)
    {
        ...
        if (e.Any(x => x.Key == "env.CONTAINER_APP_NAME" || x.Key == "env.CONTAINER_APP_REVISION")) AddOrch("Azure Container Apps", 7);
        if (e.Any(x => x.Key == "env.NOMAD_JOB_NAME" || x.Key == "env.NOMAD_ALLOC_ID")) AddOrch("Nomad", 6);
        if (e.Any(x => x.Key == "env.OPENSHIFT_BUILD_NAME" || x.Key == "env.OPENSHIFT_BUILD_NAMESPACE")) AddOrch("OpenShift", 6);
        ...
        else if (e.Any(x => x.Key.StartsWith("env.AWS_"))) { cloud = "AWS"; cloudScore = 4; }
        ...
        else if (e.Any(x => x.Key == "env.WEBSITE_SITE_NAME" || x.Key == "env.WEBSITE_INSTANCE_ID")) { cloud = "Azure"; cloudScore = 5; }
        ...
        // ── PlatformVendor ───────────────────────────────────────────────────────
        // Siemens Industrial Edge: require specific IE signals, not just generic compose or docker evidence.
        // Weak (compose-only) evidence does NOT classify as Industrial Edge.
        var ieScore = 0;
        var ieReasons = new List<ClassificationReason>();
        if (e.Any(x => x.Key.Contains("iotedge", StringComparison.OrdinalIgnoreCase) || x.Value?.Contains("iotedge", StringComparison.OrdinalIgnoreCase) == true)) { ieScore += 5; ieReasons.Add(new("IoTEdge signal detected", new[] { "environment", "runtime-api" })); }
        if (e.Any(x => x.Key.Contains("industrial", StringComparison.OrdinalIgnoreCase) || x.Value?.Contains("siemens", StringComparison.OrdinalIgnoreCase) == true)) { ieScore += 4; ieReasons.Add(new("Siemens-specific hints observed", new[] { "environment", "runtime-api" })); }
        // Compose evidence is corroboration only when combined with IE-specific signals
        if (ieScore > 0 && e.Any(x => x.Key.Contains("compose", StringComparison.OrdinalIgnoreCase))) { ieScore += 2; ieReasons.Add(new("Docker Compose corroboration", new[] { "runtime-api" })); }
