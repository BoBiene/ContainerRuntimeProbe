...
| Kubernetes workload without RBAC to read Pod/Node | service-account env, CA bundle digest, API version outcome, namespace tuple, kernel/hardware profile | L1 via weak host-profile digest when enough coarse host signals are visible | L1 via namespace tuple | L2 via cluster CA digest | none | none for environment; future API certificate corroboration could raise confidence |
| Kubernetes workload with readable Pod/Node metadata | Kubernetes API metadata, service-account CA digest, optional node UID/provider ID, namespace tuple | L3 via node UID or L2 via provider ID | L1 or L2 depending on workload source | L2 via cluster CA digest | none | Future cluster UID or API-certificate corroboration |
