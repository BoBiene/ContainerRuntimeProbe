...
| Container | Kubernetes pod/container workload tokens | `kubernetes`: `kubernetes.pod.uid`; `proc-files`: `kubernetes.cgroup.pod_uid`, `kubernetes.cgroup.container_token` | Workload | L2 with container token, else L1 | Implemented | Better Kubernetes-specific workload identity when visible; weaker workload fallbacks remain visible beside stronger runtime IDs. |
| Container | Namespace tuple | `proc-files`: `ns.pid`, `ns.mnt`, `ns.net` | Workload | L1 | Implemented | Weak restart-sensitive fallback that remains visible even when stronger workload IDs are also present. |
