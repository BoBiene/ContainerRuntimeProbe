...
| Container | Workload correlation profile | normalized workload hostname plus visible `kubernetes.pod.uid` / `kubernetes.cgroup.pod_uid`, `kubernetes.cgroup.container_token`, `ns.pid`, `ns.mnt`, `ns.net`, and Compose/stack labels when visible | Workload | L1 | Implemented | Shown as `Workload ID`. Correlation only, never a binding candidate, and hostname input remains sensitive. |
| Container | Runtime inspect container ID | `runtime-api`: `container.id` | Workload | L2 | Implemented | Shown as `Container ID` when a socket-backed inspect path is readable. |
| Container | Kubernetes pod/container workload tokens | `kubernetes`: `kubernetes.pod.uid`; `proc-files`: `kubernetes.cgroup.pod_uid`, `kubernetes.cgroup.container_token` | Workload | L2 with container token | Implemented | Kubernetes-specific stronger workload/container anchor when a per-container token is visible; weaker pod- or namespace-scoped signals remain part of `Workload ID`. |
| Environment | Diagnostic runtime/environment fingerprint | `Host.DiagnosticFingerprints[]` | Runtime / Platform | L1 | Implemented | Shown as `Runtime Profile ID` for standalone containers and as `Environment ID` for Kubernetes or industrial variants. Diagnostic only. |
...
| Generic Docker/Podman container on native Linux | CPU, memory, DMI when visible, workload hostname, namespace tuple, optional runtime inspect | L1 via host-profile digest, or L2 via explicit hardware IDs when visible | L2 via `container.id` when visible, plus L1 `Workload ID` from workload hostname and namespace/profile signals | L2 via Compose or Portainer metadata when visible, else L1 `Runtime Profile ID` | none | none for deployment identity; hypervisor remains separate |
| Container on WSL2 / Hyper-V | CPU, memory, `cpu.flag.hypervisor`, `bus.vmbus.present`, `platform.modalias`, workload hostname, namespace tuple | L1 via weak host-profile digest unless stronger host IDs become visible | L2 via `container.id` when visible, plus L1 `Workload ID` from workload hostname and namespace/profile signals | L2 via Compose or Portainer metadata when visible, else L1 `Runtime Profile ID` | none, only vendor classification | Hypervisor L2 via VM generation/UUID |
| Kubernetes workload without RBAC to read Pod/Node | service-account env, CA bundle digest, workload hostname, cgroup workload token, namespace tuple, kernel/hardware profile | L1 via weak host-profile digest when enough coarse host signals are visible | L2 via Kubernetes cgroup container token when visible, plus L1 `Workload ID` from workload hostname, pod, and namespace signals | L2 via cluster CA digest, plus L1 `Environment ID` from the weak environment fingerprint | none | none for environment; future API certificate corroboration could raise confidence |
| Kubernetes workload with readable Pod/Node metadata | Kubernetes API metadata, service-account CA digest, workload hostname, pod UID, optional node UID/provider ID, cgroup workload token, namespace tuple | L3 via node UID or L2 via provider ID | L2 via pod UID plus cgroup container token when visible, plus L1 `Workload ID` from workload hostname, pod, and namespace/profile signals | L2 via cluster CA digest, plus L1 `Environment ID` from the weak environment fingerprint | none | Future cluster UID or API-certificate corroboration |
| Cloud VM container with reachable IMDS | cloud instance metadata, cloud account/project/subscription/compartment metadata, cloud region, machine type, workload hostname, namespace tuple | L3 via cloud instance ID | L1 or L2 depending on runtime visibility, with `Workload ID` available from workload hostname/profile signals | L2 via cloud provider boundary digest when visible, else L1 `Runtime Profile ID` for standalone containers | none | Provider-specific corroboration if stronger environment certainty is ever needed |
...
- `Workload` L1 via workload hostname and namespace/profile signals when those local signals are visible
- `Environment` L1 as `Runtime Profile ID` for standalone containers or as weak `Environment ID` for Kubernetes-style summaries
