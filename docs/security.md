...
- Current anchor values are digests over observed cloud instance IDs, Kubernetes node identities, matched Siemens IED certificate-chain runtime evidence, or local host machine-id sources such as Windows `MachineGuid` or Linux `machine-id`, not raw IDs.
