...
- Current anchor values are digests over observed cloud instance IDs, Kubernetes node identities, matched Siemens IED certificate-chain runtime evidence, local host machine-id sources such as Windows `MachineGuid` or Linux `machine-id`, or explicit runtime inspect container IDs, not raw IDs.
