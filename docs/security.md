...
- Current anchor values are digests over observed cloud instance IDs, Kubernetes node identities, visible Kubernetes service-account CA bundles, matched Siemens IED certificate-chain runtime evidence, local host machine-id sources such as Windows `MachineGuid` or Linux `machine-id`, explicit host-visible hardware identifiers such as SMBIOS or SoC serials, weak public host-profile signals, or explicit runtime inspect container IDs, not raw IDs.
