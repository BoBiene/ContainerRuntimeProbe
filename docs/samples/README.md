...
1. A user opens a prefilled GitHub issue with `container-runtime-probe sample` or `docker run --rm ghcr.io/bobiene/container-runtime-probe:latest sample`.
2. The user optionally attaches `my-report.json` created with `docker run --rm ghcr.io/bobiene/container-runtime-probe:latest json > my-report.json`.
